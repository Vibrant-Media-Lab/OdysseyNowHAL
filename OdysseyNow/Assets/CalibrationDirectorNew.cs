...
public class CalibrationDirectorNew : MonoBehaviour {
    ...
    [Header("GUI related")] public UnityEngine.UI.Text mTextInstruction;
    ...
    [Header("non-editor purposed")] public CalibrationOdysseySettings mCalibrationSettings;
    ...
    private void Start() {
        ...
        if (mCalibrationSettings.p1_read || mCalibrationSettings.p2_read && PlayerPrefs.GetInt("console_connected") == 1) {
            ...
            if (mCalibrationSettings.useOverlay) {
                ...
                mStepPipe.Add(new S3_CalibRead(this, "The top-left conner of the tv screen", mCalibTV_topLeft,
                                               playerReadName, "top-left"));
                mStepPipe.Add(new S3_CalibRead(this, "The bottom-right conner of the tv screen", mCalibTV_bottomRight,
                                               playerReadName, "bottom-right"));
            }
        }
        ...
        if (!mCalibrationSettings.p1_read || !mCalibrationSettings.p2_read && PlayerPrefs.GetInt("console_connected") == 1) {
            ...
            if (mCalibrationSettings.useOverlay) {
                ...
                // no screen overlay is used, calibrate to the top-left and bottom-right of the tv screen
                mStepPipe.Add(new S4_CalibWrite(this, "the top-left conner of the tv screen", mCalibTV_topLeft,
                                                playerWriteName, "top-left"));
                mStepPipe.Add(new S4_CalibWrite(this, "the bottom-right conner of the tv screen", mCalibTV_bottomRight,
                                                playerWriteName, "bottom-right"));
            }
        }

        // So far the camera3DScene.transform remains the "target" transform
        mStepPipe.Add(new S0_2D3DAnimation(this, camera3DScene, cameraInGame));
    ...
    private void StateTransit(IStateStep targetState) {
...
class S0_2D3DAnimation : IStateStep {
    ...
    void IStateStep.OnEnterStep() {
        ...
        var fromTransform = mCamFrom.transform;
        var toTransform = mCamTo.transform;
        mPosFrom = fromTransform.position;
        mPosTo = toTransform.position;
        mRotFrom = fromTransform.rotation;
        mRotTo = toTransform.rotation;
    ...
    void IStateStep.Step() {
        ...
        mDirc.camera3DScene.transform.position = Vector3.Slerp(mPosFrom, mPosTo, progress);
        mDirc.camera3DScene.transform.rotation = Quaternion.Slerp(mRotFrom, mRotTo, progress);
...
class S1_Devices : IStateStep {
    ...
    void IStateStep.OnEnterStep() {
        if (mDirc.mCalibrationSettings.p1_read && mDirc.mCalibrationSettings.p2_read) {
            mDirc.mTextInstruction.text = "Please connect both <color=brown>P1</color> and <color=brown>P2</color> " +
                                          "controllers";
        }
        else if (mDirc.mCalibrationSettings.p1_read) {
            mDirc.mTextInstruction.text = "Please setup your Oddyssey with as follows: \n" +
                                          "connect <color=brown>P1</color>, " + "disconnect <color=brown>P2</color>\n";
        }
        else if (mDirc.mCalibrationSettings.p2_read) {
            mDirc.mTextInstruction.text = "Please setup your Oddyssey with as follows: \n" +
                                          "disconnect <color=brown>P1</color>, " + "connect <color=brown>P2</color>\n";
        }
        else {
            mDirc.mTextInstruction.text =
                "Please disconnect both <color=brown>P1</color> and <color=brown>P2</color> " + "controllers";
        }
...
class S2_Overlay : IStateStep {
    ...
    void IStateStep.OnEnterStep() {
        if (mOverlaySpot == null) {
            mDirc.mTextInstruction.text = "Please put the screen overlay on the TV screen.";
        }
        else {
            mDirc.mTextInstruction.text = "Please move " + mAvailablePlayerName + " to the spot.";
    ...
    public S2_Overlay(CalibrationDirectorNew calibrationDirector, string extraText) {
        mDirc = calibrationDirector;
...
class S3_CalibRead : IStateStep {
    ...
    void IStateStep.OnEnterStep() {
        if (mOverlaySpot.GetComponent<SpriteMask>() == null) {
            mDirc.mTextInstruction.text = "Please use the Odyssey controller to \n" + "move " + mAvailablePlayerName +
                                          " to " + mPositionText;
        }
        else {
            mDirc.mTextInstruction.text = "Please use the Odyssey controller to \n" + "move " + mAvailablePlayerName +
    ...
    void IStateStep.OnExitStep() {
        ...
        if (mHWData == null) {
            Debug.Log("Controller is probably not connected");
        }
        ...
        if (mOverlaySpot.GetComponent<SpriteMask>() != null) {
            ...
            var position = mOverlaySpot.transform.position;
            switch (mMode) {
                case "top-left":
                    mDirc.mCalibrationSettings._calib_unity_x_left = position.x;
                    mDirc.mCalibrationSettings._calib_unity_y_top = position.y;
                    break;
                case "bottom-right":
                    mDirc.mCalibrationSettings._calib_unity_x_left = position.x;
                    mDirc.mCalibrationSettings._calib_unity_y_top = position.y;
        ...
        switch (mMode) {
            case "top-left" when mAvailablePlayerName.ToUpper() == "P1":
                ...
                break;
            case "top-left" when mAvailablePlayerName.ToUpper() == "P2":
                ...
                break;
            // Save the current calibration-read
            case "bottom-right" when mAvailablePlayerName.ToUpper() == "P1":
                ...
                break;
            case "bottom-right" when mAvailablePlayerName.ToUpper() == "P2":
    ...
    void IStateStep.Step() {
        ...
        mHWData = mDirc.consoleMirror.readControllerRawData();
        if (mHWData != null) {
            mDirc.textDBP1.text = $"({mHWData.P1_X_READ},{mHWData.P1_Y_READ})";
            mDirc.textDBP2.text = $"({mHWData.P2_X_READ},{mHWData.P2_Y_READ})";
        }
...
class S4_CalibWrite : IStateStep {
    ...
    void IStateStep.OnEnterStep() {
        if (mOverlaySpot.GetComponent<SpriteMask>() == null) {
            mDirc.mTextInstruction.text = "Please use the Keyboard to \n" + "move " + mAvailablePlayerName + " to " +
                                          mPositionText;
        }
        else {
            mDirc.mTextInstruction.text =
        ...
        switch (mMode) {
            case "top-left": {
                ...
                mDirc.mCalibrationSettings._calib_p2_write_votage_x_left =
                    mDirc.consoleMirror.xConvertToConsole(position.x);
                mDirc.mCalibrationSettings._calib_p2_write_votage_y_top =
                    mDirc.consoleMirror.yConvertToConsole(position.y);
                break;
            }
            case "bottom-right": {
                var p = mAvailablePlayerName == "P1" ? mDirc.p1_target : mDirc.p2_target;
